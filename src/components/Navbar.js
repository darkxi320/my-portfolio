...
function Navbar() {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 text-white bg-black px-4 fixed">
      <div className=" text-4xl font-signature ml-2 cursor-pointer ">Ahmed</div>

      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className="px-4 cursor-pointer captilize font-medium text-gray-50 hover:scale-90 duration-200"
          >
